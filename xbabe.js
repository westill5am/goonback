...
module.exports = async function xbabe(query) {
  ...
  try {
    ...
    let videoEls = $('.video-block, .video-item, .thumb, .video, .item');
    if (videoEls.length === 0) {
      videoEls = $('a[href*="/video/"]'); // fallback: links to videos
    }

    videoEls.each((i, el) => {
      let title = $(el).find('.video-title').text().trim() || $(el).find('.title').text().trim() || $(el).attr('title') || $(el).find('img').attr('alt') || $(el).text().trim();
      ...
      // Normalize href
      if (href && !href.startsWith('http')) href = 'https://xbabe.com' + href;
      // Normalize preview
      if (preview && preview.startsWith('//')) preview = 'https:' + preview;
      if (preview && preview.startsWith('/')) preview = 'https://xbabe.com' + preview;
    ...
    if (results.length === 0) {
      console.error('XBabe: No results found. Selectors may be outdated or site structure changed.');
    }
  } catch (err) {

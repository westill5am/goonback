...
module.exports = async function sexvid(query) {
  const results = [];
  const maxPages = 3;
  
  try {
    for (let page = 1; page <= maxPages; page++) {
      const searchUrl = `https://www.sexvid.xxx/search?q=${encodeURIComponent(query)}&page=${page}`;
      ...
      $('.video-item, .thumb-item, .video, .item').each((i, el) => {
        ...
        if (title && href) {
          ...
          if (href.startsWith('/')) {
            href = 'https://www.sexvid.xxx' + href;
          }
          
          if (preview && !preview.startsWith('http')) {
            preview = preview.startsWith('/') ? 'https:' + preview : 'https://www.sexvid.xxx/' + preview;
          }
    ...
    console.error("sexvid error:", err.message);
  }

...
module.exports = async function pornjam(query) {
  const results = [];
  const maxPages = 3;
  
  try {
    for (let page = 1; page <= maxPages; page++) {
      const searchUrl = `https://www.pornjam.com/search?q=${encodeURIComponent(query)}&page=${page}`;
      ...
      $('.video-item, .thumb, .video-thumb, .item').each((i, el) => {
        ...
        if (title && href) {
          ...
          if (href.startsWith('/')) {
            href = 'https://www.pornjam.com' + href;
          }
          
          if (preview && !preview.startsWith('http')) {
            preview = preview.startsWith('/') ? 'https:' + preview : 'https://www.pornjam.com/' + preview;
          }
    ...
    console.error("pornjam error:", err.message);
  }

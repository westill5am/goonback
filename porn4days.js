...
module.exports = async function porn4days(query) {
  const results = [];
  const maxPages = 3;
  
  try {
    for (let page = 1; page <= maxPages; page++) {
      const searchUrl = `https://www.porn4days.com/search/${encodeURIComponent(query)}?p=${page}`;
      ...
      $('.video-item, .thumb-item, .video, .item').each((i, el) => {
        const $el = $(el);
        ...
        if (title && href) {
          ...
          if (href.startsWith('/')) {
            href = 'https://www.porn4days.com' + href;
          }
          
          if (preview && !preview.startsWith('http')) {
            preview = preview.startsWith('/') ? 'https:' + preview : 'https://www.porn4days.com/' + preview;
          }
      ...
      if ($('.video-item, .thumb-item, .video, .item').length === 0) {
        break;
      }
    }
  } catch (err) {
    console.error("porn4days error:", err.message);
  }

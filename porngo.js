...
module.exports = async function porngo(query) {
  const results = [];
  const maxPages = 3;
  
  try {
    for (let page = 1; page <= maxPages; page++) {
      const searchUrl = `https://www.porngo.com/search?k=${encodeURIComponent(query)}&p=${page}`;
      ...
      $('.video-item, .thumb-item, .video, .item').each((i, el) => {
        ...
        if (title && href) {
          ...
          if (href.startsWith('/')) {
            href = 'https://www.porngo.com' + href;
          }
          
          if (preview && !preview.startsWith('http')) {
            preview = preview.startsWith('/') ? 'https:' + preview : 'https://www.porngo.com/' + preview;
          }
    ...
    console.error("porngo error:", err.message);
  }

...
try {
  ...
  console.log('Scrapers loaded successfully:', Object.keys(scrapers).length);
} catch (error) {
...
// Serve static files from this backend's /public folder (e.g., for favicon)
app.use(express.static(path.join(__dirname, 'public'))); // Serves /goonerbrain.png from public/goonerbrain.png

// Serve static files from the goonerbrain-main directory (e.g., for images/Stake_Casino_728x90.gif)
// This makes 'goonerbrain-main' the root for these static assets.
// So, goonerbrain-main/images/foo.gif becomes accessible at /images/foo.gif
app.use(express.static(path.join(__dirname, '..', 'goonerbrain-main')));

// REMOVE old template serving
...
app.get('/', (req, res) => {
  // Serve the index.html from the goonerbrain-main directory
  res.sendFile(path.join(__dirname, '..', 'goonerbrain-main', 'index.html'));
});

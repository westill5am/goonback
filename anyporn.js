...
module.exports = async function anyporn(query) {
  const results = [];
  const maxPages = 3;
    try {
    for (let page = 1; page <= maxPages; page++) {
      const searchUrl = `https://www.anyporn.com/search/?q=${encodeURIComponent(query)}&page=${page}`;
      ...
      $('.video-item, .thumb-item, .video, .item').each((i, el) => {
        ...
        if (title && href) {
          ...
          if (href.startsWith('/')) {
            href = 'https://www.anyporn.com' + href;
          }
          
          if (preview && !preview.startsWith('http')) {
            preview = preview.startsWith('/') ? 'https:' + preview : 'https://www.anyporn.com/' + preview;
          }
    ...
    console.error("anyporn error:", err.message);
  }
